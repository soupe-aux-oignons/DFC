...
def IDinsertion(table,face_path,results,idface):#assume insertRow works
    ...
    if myresult!=[]:#flag
        for record in myresult:#parcours les chemin et les identités des entrées dans la table, flag
            print("this is my record :",record)
            comparaisons = comparateurDlib(face_path, record[0],'distance')#compare le visage 'face_path' avec le visage du chemin de l'entrée 'record'
            if comparaisons!=[]:
                for check in comparaisons:
                    ...
                    if check[0]<=0.5:
                        ...
                        return
    results.append(idface)#ajoute aux valeurs de 'face_path' l'identité unknown
    insertRow(table,colums,tuple(results))#insère le visage et ses données dans la table
...
def extractFace(detector,input_path,output_path,importance,margin=0,idface="unknown"):
    ...
    faces_paths=load(output_path)#flag
    ...
    for img_path in img_paths :
        ...
        for result_dic in results :
            ...
            if  sizeAreaRatioCheck(img_save,face,importance) :
                ...
                #print("liste de résultat",result_list,"\n")
                #insertion dans la table du visage extrait
                print(IDinsertion(faceTable,f"{output_path}/face.{i}.jpg",result_list,idface))
                i+=1
...
if __name__=="__main__":
  ...
  #print(comparateurDlib(load(analysis_path)[0],load(output_path)[0],"distance"))
