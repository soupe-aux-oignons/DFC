...
def IDinsertion(table,face_path,results,idface,j):
    ...
    if myresult!=[]:
        #si la liste n'est pas vide, parcours les chemin et les identités des entrées dans la table
        for record in myresult:
            ...
            if comparaisons!=[]:
                for check in comparaisons:
                    if check[0]<=0.5:
                        #si la distance entre les visages <=0.5 ajoute aux données de 'face_path' l'identité du visage de la table
                        results.append(record[1])
                        insertRow(table,colums,tuple(results))#insère le visage et ses données dans la table
                        return 
    results.append(idface+str(j))#sinon ajoute aux valeurs de 'face_path' l'identité unknown
    insertRow(table,colums,tuple(results))#insère le visage et ses données dans la table
    return (idface)
